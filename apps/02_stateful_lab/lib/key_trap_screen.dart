...
/// Thí nghiệm 3 — vì sao xoá một phần tử lại làm state nhảy sang widget khác.
///
/// Hai cột giống hệt nhau, cùng danh sách nhãn. Khác đúng một thứ: cột phải có
/// `ValueKey`. Xoá phần tử đầu rồi so hai cột là thấy ngay.
    ...
    return Column(
      ...
      children: [
        ...
        for (final label in labels)
          _ColorTile(
            // Toàn bộ khác biệt của thí nghiệm nằm ở đúng dòng này.
            key: useKeys ? ValueKey(label) : null,
            label: label,
          ),
      ],
    );
...
/// Con trỏ chạy trong bảng màu. Mỗi `State` mới lấy một màu mới, nên nhìn màu
/// là biết `State` nào được tạo mới và `State` nào được giữ lại.
int _colorCursor = 0;
...
class _ColorTileState extends State<_ColorTile> {
  ...
  void initState() {
    super.initState();
    // Chạy đúng một lần cho mỗi State. Màu này là thứ phân biệt "State cũ được
    // giữ lại" với "State mới được tạo".
